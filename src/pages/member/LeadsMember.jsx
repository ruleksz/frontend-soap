      ...
      <div className="overflow-x-auto bg-white shadow rounded-lg">
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={7} className="text-center p-6 text-gray-500">Memuat data...</td>
              </tr>
            ) : leads.length > 0 ? (
              leads.map((cabuy, index) => {
                const idCabuy = cabuy?.id_cabuy ?? cabuy?.id;
                return (
                  <tr key={idCabuy ?? `cabuy-${index}`} className="hover:bg-blue-50">
                    <td className="p-3 border-b">{index + 1}</td>
                    <td className="p-3 border-b">{cabuy.nama_cabuy}</td>
                    <td className="p-3 border-b">{cabuy.kontak}</td>
                    <td className="p-3 border-b">{cabuy.tanggal_follow_up ? new Date(cabuy.tanggal_follow_up).toLocaleDateString("id-ID") : "-"}</td>
                    <td className="p-3 border-b">{cabuy.tanggal_masuk ? new Date(cabuy.tanggal_masuk).toLocaleDateString("id-ID") : "-"}</td>
                    <td className="p-3 border-b">
                        <span
                            className={`px-3 py-1 rounded-full text-sm ${
                            cabuy.status === "Closing"
